...
agent_communication:
  ...
  - agent: "main"
    message: "CORREZIONI ULTERIORI PROBLEMI RISOLTI - Implementate le ultime correzioni richieste: 1) RISOLTO problema caratteri all'inverso nella chat - aggiunto CSS specifico per textarea chat (.chat-textarea) con direction: ltr, text-align: left e unicode-bidi: normal per evitare conflitti con le transizioni globali 2) RISOLTO problema creazione capo promoter senza organizzazione - modificato backend per rendere il campo organizzazione opzionale nel modello TemporaryCredentials, aggiornata API per gestire organizzazione None, modificato frontend per mostrare chiaramente che l'organizzazione è opzionale. Ora si può creare un capo promoter e assegnare l'organizzazione successivamente. Tutti i test backend passati al 100%."
  - agent: "testing"
